...
Versión: 3.0 (release AWS y cierre de flujos de identidad, imágenes y administración)
Estado: Auditado e implementado. v2.1–2.9 como antes. **v3.0:** despliegue real en AWS (ECS Fargate + ALB, ECR, RDS, S3/CloudFront), builds móviles firmados, registro obligatorio con género/imágenes, matching por género, cambio passenger↔driver, aprobación de cambios de perfil, correo SMTP opcional, eliminación lógica de usuarios/coordinadores, correcciones SPA/Maps/logo/SOS.
...
| Deliverable | Stack |
|---|---|
| `backend/` | C# .NET 10 Web API, EF Core, PostgreSQL 16 |
| `web/` | React 19 + TypeScript, Vite 8, TanStack Query, Tailwind |
...
La plataforma tiene exactamente **cuatro roles**. Cada sesión mantiene un único rol activo. Una cuenta móvil puede cambiar de passenger a driver mediante solicitud con datos e imagen del vehículo, y de driver a passenger de forma directa; el cambio revoca sesiones para emitir claims consistentes en el siguiente login. No hay rol de guardia de seguridad en v1: las alertas SOS las maneja el coordinador.
...
|---|---|---|---|---|
| CRUD universities / campuses / coordinador accounts (incluye borrado lógico de coordinadores) | Yes | No | No | No |
| Global platform stats (`/super/stats`) | Yes | No | No | No |
| Approve/deny registrations/profile changes, block/unblock/delete users | No (out of tenant ops) | Yes (own university) | No | No |
...
| Profile + emergency contacts | No | No | Direct edit | Change request approved by coordinador |
| Switch passenger ↔ driver mode | No | No | To passenger directly | To driver after vehicle approval |
...
flowchart LR
  ...
  subgraph aws [AWS - test account]
    CF[CloudFront - HTTPS entrypoint]
    ALB[Application Load Balancer]
    API[.NET 10 Web API - ECS Fargate]
    DB[(RDS PostgreSQL 16)]
    S3[S3 private bucket - static web]
    ECR[ECR - backend image]
    LOGS[CloudWatch Logs]
  end
  GMaps[Google Maps Platform]

  Mobile -->|REST + JWT, polling| CF
  Web -->|HTTPS| CF
  CF -->|SPA/assets| S3
  CF -->|/api/*| ALB
  ALB --> API
...
- **Backend**: .NET 10 Web API, capas `Api / Application / Domain / Infrastructure`, EF Core + Npgsql, JWT bearer auth, Swagger/OpenAPI y logging en CloudWatch. La imagen `linux/amd64` se almacena en ECR y corre en ECS Fargate detrás de un ALB. Google Directions API se llama **server-side** al publicar el trip.
- **Web**: Vite 8 + React 19 + TS, React Router (rutas `/login`, `/super/*`, `/admin/*`), TanStack Query para datos y polling, Tailwind v4. El build estático vive en S3 privado y CloudFront aplica el fallback SPA mediante una CloudFront Function; `/api/*` se enruta al ALB.
...
- **Artefactos móviles**: GitHub Actions produce un APK Android firmado de forma estable y una aplicación ZIP para iOS Simulator. El iPhone físico requiere firma/provisioning Apple o distribución TestFlight.
...
| `users` | university_id (null for super_admin), campus_id (null for super_admin and coordinador), role (super_admin/coordinador/driver/passenger), status (active/blocked/pending/deleted), name, email, password_hash (BCrypt), gender (male/female), profile_image (data URL validada), career, id_number, rating_avg, eco_balance, eco_lifetime, must_change_password |
| `refresh_tokens` | user_id, token_hash, expires_at, revoked_at (nullable) |
| `registration_requests` | university_id, campus_id, name, email, password_hash/marker, role, gender, profile_image, career, id_number, vehicle_json, status, decided_by/at. También modela solicitudes de cambio de perfil, vehículo y modo conductor |
| `vehicles` | university_id, user_id, make_model, plate, color, seats_total, image (obligatoria) |
...
- Matching por género: una solicitud de viaje solo puede crearse cuando conductor y pasajero tienen género completo y coincidente.
- Borrado lógico: eliminar usuarios/coordinadores revoca refresh tokens, anonimiza PII y conserva FKs/auditoría; usuarios eliminados no aparecen en listados ni pueden autenticarse.
- Perfil y rol: el pasajero envía cambios de perfil al coordinador para aceptar/denegar; passenger→driver requiere vehículo completo e imagen, mientras driver→passenger es directo.
...
| Super admin | `GET/POST/PUT /super/universities` · `POST /super/universities/{id}/suspend` · `GET/POST/PUT/DELETE /super/universities/{id}/campuses` · `GET/POST /super/universities/{id}/coordinadores` · `DELETE /super/coordinadores/{id}` · `POST /super/coordinadores/{id}/reset-password` · `GET /super/stats` |
| Admin: users | `GET /admin/registration-requests` (incluye alta, perfil, vehículo y modo) · `POST /admin/registration-requests/{id}/accept|deny` · `GET /admin/users` · `POST /admin/users/{id}/block|unblock` · `DELETE /admin/users/{id}` · `GET /admin/users/export` |
...
| Profile / mode | `GET/PUT /me/profile` (edición directa solo donde aplica) · `POST /me/profile-change` (pasajero, aprobación) · `POST /me/mode` (passenger↔driver) · `GET/POST/DELETE /me/emergency-contacts` · `GET/PUT /me/vehicle` |
...
- Registro: carrera se elige desde catálogo UTN searchable; nombre, correo, contraseña, universidad, campus, carrera, cédula, género e imagen de perfil son obligatorios. Un conductor además entrega todos los datos e imagen del vehículo.
- Email transaccional opcional: aceptación/denegación de solicitudes usa SMTP cuando está configurado; una falla de correo no revierte la decisión ya persistida.
...
| `/super/universities/:id` | University detail | Campuses CRUD con búsqueda Google Maps; coordinadores list/create/reset/delete |
...
| `/admin/users` | Gestión de Usuarios | Cola de altas/cambios (accept/deny), directorio, block/unblock/delete, CSV export |
...
|---|---|---|
| Login / Register | (new, follows design language) | Branding UTN; universidad/campus; carrera searchable; género, cédula e imagen obligatorios; vehículo + imagen obligatorios para driver |
| Passenger: Inicio, Mis Viajes, Perfil, Ayuda | `PaxHome/PaxTrips/PaxProfile/PaxHelp` | Viajes/ECT/CO₂ reales; cambios de perfil por solicitud al coordinador; solicitud de modo driver con vehículo |
| Driver: Inicio, Mis Viajes, Perfil, Ayuda | `DrvHome/DrvTrips/DrvProfile/DrvHelp` | Requests accept/reject; mapa de waypoints; edición de perfil/vehículo; cambio directo a modo passenger |
...
| SOS overlay | `SOSOverlay` | `POST /sos` con GPS; si la alerta fue resuelta externamente, el overlay se cierra sin bloquear la navegación |
...
Cloud real (AWS cuenta de prueba `900103507605`, stack `kubix-test`):
- **ECR** `kubix-test-api` almacena imágenes `linux/amd64` con scanning y retención de cinco imágenes.
- **ECS Fargate** ejecuta la API en el cluster/service `kubix-test` / `kubix-test-api`; **Application Load Balancer** y target group realizan health check en `/api/v1/health`. App Runner queda como fallback opcional, pero no estaba habilitado en la cuenta.
- **RDS PostgreSQL 16** `db.t4g.micro`, 20 GiB gp3; migraciones EF Core al inicio del contenedor.
- **S3 privado** aloja el build web; **CloudFront** `https://d2dgmlbp00gdhh.cloudfront.net` es el único punto público HTTPS, sirve la SPA, enruta `/api/*` al ALB y usa CloudFront Function para deep links.
- **CloudWatch Logs**, IAM roles, security groups, default VPC/subnets y Origin Access Control completan la plataforma.
- Mobile: GitHub Actions publica APK Android release con keystore estable y ZIP de iOS Simulator, ambos configurados con CloudFront y Google Maps.
- CI/CD: `pr-tests.yml` ejecuta cobertura backend/web/mobile; `deploy.yml` soporta dispatch manual o push con `DEPLOY_ON_PUSH=true` y secrets AWS; `mobile-builds.yml` puede ejecutarse de forma independiente.
- Validación real: health, rutas SPA, logo UTN y guardas API pasaron `deploy/scripts/smoke.sh`.

---

## 9. Suite de testing y herramientas de QA

| Concern | Tool | Estado real |
|---|---|---|
| Unit tests BE | xUnit + cobertura .NET | Hecho; gate ≥70% y artefacto CI |
| Unit tests web | Vitest + React Testing Library | Hecho; gate ≥70% y artefacto CI |
| Unit tests mobile | flutter_test | Hecho; gate ≥60% y artefacto CI |
| Service tests + BE load | Postman + Newman | Pendiente KBX-28 |
| Performance | JMeter | Pendiente KBX-28 |
| Functional automation | Selenium | Pendiente KBX-29 |
| Code analysis | SonarCloud | Pendiente KBX-29 |
| Security / SAST | Semgrep + dependency audits | Pendiente KBX-29 |
| Test management | TestLink | Pendiente KBX-29 |
| Defect management | MantisBT | Pendiente KBX-29 |
...
Plataforma multi-tenant de seguridad para carpooling (super admin → universidades → campuses) con React 19 admin web, .NET 10 API, Flutter mobile, PostgreSQL, tracking en vivo con Google Maps, alertas SOS, tooling QA y deployment AWS.
...
| **T7** | Deploy — AWS + CI/CD | Media | 2026-07-17 | 2026-07-19 | Deploy | Hecho y desplegado |
...
- [x] Login/logout/guards; cambio forzado de password
- [x] Super: CRUD universidades/campuses/coordinadores + stats; eliminación lógica de coordinadores
- [x] Campus: picker de mapa (Maps JS) al crear/editar ubicación (v2.9 / KBX-15)
- [x] Panel: KPIs, SOS poll + resolve, XP por carrera
- [x] Usuarios: cola accept/deny (alta/perfil/vehículo/modo), filtros, block/unblock/delete, export
...
- [x] Login + registro wizard obligatorio (carrera searchable, género, cédula, imagen; vehículo con imagen para driver) + pending; token seguro + refresh
...
- [x] Cambio passenger↔driver; cambios de perfil passenger sujetos a aprobación
- [x] SOS remoto resuelto cierra overlay sin dejar al usuario atrapado
...
- [x] API: imagen ECR → ECS Fargate + ALB; migraciones al arranque; logs CloudWatch
- [x] Web: S3 privado + CloudFront + CloudFront Function (SPA routing y `/api/*`)
- [x] CI: PR tests (`pr-tests.yml`); main/manual deploy + invalidation (`deploy.yml`)
- [x] `deploy/scripts/smoke.sh` verde contra la cuenta real
- [x] Script/doc de teardown sin recursos facturables residuales

**Notas:**
Infra de prueba AWS desplegada: ECR, ECS Fargate + ALB, RDS PostgreSQL 16 `db.t4g.micro`, S3 privado + CloudFront/Function, CloudWatch Logs e IAM. App Runner quedó solo como fallback porque la cuenta no tenía el servicio habilitado. Detalle: PLAN.md → KBX-30 · `deploy/README`.

**Datos adjuntos:**
- Link: `deploy/README` (deploy + teardown)
- Link: workflows `.github/workflows/*`
- Link: CloudFront `https://d2dgmlbp00gdhh.cloudfront.net` + ALB/ECS en outputs CloudFormation
...
**Descripción:** Crear `smart-carpool-security/` con `backend/` (solución .NET 10: Api, Application, Domain, Infrastructure, Tests), `web/` (Vite 8 + React 19 + TS + Tailwind), `mobile/` (Flutter con Riverpod/Dio), `qa/`, `deploy/`, Docker Compose, `.env.example` y README reproducible.
...
**Descripción:** CRUD universities (+suspend bloquea logins), CRUD campuses con ubicación Google Maps, crear coordinador (password temporal y cambio forzado), reset y eliminación lógica de coordinador (anonimiza y revoca sesiones), `GET /super/stats`.
...
**Descripción:** `POST /auth/register` valida university/campus, dominio opcional y todos los campos obligatorios (carrera UTN, cédula, género, imagen; vehículo + imagen para driver); accept/deny con email SMTP opcional; directorio admin con filtros, block/unblock/delete lógico y export. Incluye solicitudes de cambio de perfil del pasajero y passenger→driver; driver→passenger directo; contactos de emergencia y matching de viaje por género.
...
**QA:** Vitest: route guards por rol; round trip login/logout contra API local; deep link no autorizado → login.

### KBX-15 — Web: consola de super admin
**Descripción:** tabla `/super/universities` + diálogos create/edit/suspend; detalle con CRUD de campuses (búsqueda/selección Google Maps) y coordinadores (crear, resetear, eliminar); KPI cards `/super/stats`.
...
**Descripción:** Portar Figma `UsersScreen`: cola unificada de altas/cambios de perfil/vehículo/modo (accept/deny), cards de stats, directorio con filtros/paginación, block/unblock, eliminación lógica y export CSV.
...
**Descripción:** Portar Figma `SecurityScreen`: 4 KPI de resumen, tabla de audit log con filtros type/severity y exportación completa en PDF desde el servidor.
...
**Descripción:** App Flutter con branding UTN, login y wizard de registro: university/campus, role, carrera searchable, cédula, género e imagen de perfil obligatorios; vehículo e imagen obligatorios para driver. Incluye pending approval, almacenamiento seguro, refresh y shell de navegación por rol.
...
**Descripción:** `deploy/`: CloudFormation para ECR, S3/OAC, CloudFront/Function, RDS PostgreSQL 16, IAM, ECS Fargate, ALB, security groups y CloudWatch; scripts `deploy.sh` / `teardown.sh` / `smoke.sh`; workflows de deploy, PR tests y artefactos móviles. App Runner se intenta como opción y ECS es el runtime real cuando no está habilitado.
**QA:** despliegue real completado; CloudFront `https://d2dgmlbp00gdhh.cloudfront.net`; smoke verde para health, deep links SPA, logo UTN y guardas `/api`; backend/web/mobile verdes en Actions.
**Estado:** hecho y desplegado en AWS mediante el profile local `kx-dev`; el deploy desde GitHub requiere configurar secrets AWS.
...
**Canje/redención** de EcoToken, pagos reales, chat, verificación documental avanzada, push notifications, feed de notificaciones mobile, publicación en stores, i18n, websockets, browsing cross-campus, return trips desde campus, impersonación de tenants, notificación automática de emergency contacts y rol de guardia. Una cuenta móvil sí puede alternar passenger↔driver, pero nunca mantiene ambos roles activos simultáneamente.
...
| Carga de polling por pings | Índice last-ping-per-user; intervalo configurable; JMeter pendiente en KBX-28 |
| Secret JWT no inyectado por el deploy actual | Cablear `Jwt__Key` desde Secrets Manager/GitHub antes de producción; rotar el valor actual |
| Maps web ausente en un runner cloud limpio | Pasar `VITE_GOOGLE_MAPS_API_KEY` al build o ejecutar `make sync-env` |
| Infraestructura solo de prueba | Crear stack de producción separado con RDS privada, backups, TLS ALB y políticas de retención |
